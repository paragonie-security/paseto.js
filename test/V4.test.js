...
describe('Protocol V4', () => {
    ...
    describe('keygen', () => {
        ...
        it('should generate a symmetric key - callback api', (done) => {
            V4.symmetric((err, sk) => {
                ...
                assert.equal(V4.sklength(), Buffer.byteLength(sk.raw()));
    ...
    describe('authenticated encryption', () => {
        ...
        before((done) => {
            ...
            key = new Paseto.SymmetricKey.V4();
        ...
        describe('text', () => {
            ...
            it('should encrypt and decrypt successfully - async/await api', (done) => {
                ...
                (async () => {
                    ...
                    const data  = await V4.decrypt(token, key, '');
                    assert.equal(typeof data, 'string');
                    assert.equal(data, message);
            ...
            it('should encrypt and decrypt successfully with footer - promise api', (done) => {
                ...
                V4.encrypt(message, key, footer)
                    .then((token) => {
                        ...
                        return V4.decrypt(token, key, footer);
                    })
                    .then((data) => {
            ...
            it('should encrypt and decrypt successfully with footer - async/await api', (done) => {
                ...
                (async () => {
                    const token = await V4.encrypt(message, key, footer);
                    assert.equal(typeof token, 'string');
                    assert.equal(token.substring(0, 9), 'v4.local.');
                    assert.equal(token.substring(token.length - 8, token.length), encoded_footer);

                    const data  = await V4.decrypt(token, key, footer);
                    assert.equal(typeof data, 'string');
                    assert.equal(data, message);
        ...
        describe('errors', () => {
            ...
            it('should error on decryption with an invalid key version - async/await api', (done) => {
                ...
                (async () => {
                    const token = await V4.encrypt('test', key, '');
                    assert.ok(token);
    ...
    describe('signing', () => {
        ...
        before((done) => {
            ...
            sk = new Paseto.PrivateKey.V4();
            sk.inject(Buffer.from(keypair.privateKey, 'binary'), (err) => {
                if (err) { return done(err); }

                pk = new Paseto.PublicKey.V4();
        ...
        describe('text', () => {
            ...
            it('should sign and verify successfully - async/await api', (done) => {
                ...
                (async () => {
                    const token = await V4.sign(message, sk, '');
                    assert.equal(typeof token, 'string');
                    assert.equal(token.substring(0, 10), 'v4.public.');

                    const data  = await V4.verify(token, pk, '');
                    assert.equal(typeof data, 'string');
                    assert.equal(data, message);
            ...
            it('should sign and verify successfully with footer - promise api', (done) => {
                ...
                V4.sign(message, sk, footer)
                    .then((token) => {
                        ...
                        return V4.verify(token, pk, footer);
                    })
                    .then((data) => {
            ...
            it('should sign and verify successfully with footer - async/await api', (done) => {
                ...
                (async () => {
                    const token = await V4.sign(message, sk, footer);
                    assert.equal(typeof token, 'string');
                    assert.equal(token.substring(0, 10), 'v4.public.');
                    assert.equal(token.substring(token.length - 8, token.length), encoded_footer);

                    const data  = await V4.verify(token, pk, footer);
                    assert.equal(typeof data, 'string');
                    assert.equal(data, message);
        ...
        describe('errors', () => {
            ...
            it('should error on verifying with an invalid key version - async/await api', (done) => {
                ...
                (async () => {
                    const token = await V4.sign('test', sk, '');
                    assert.ok(token);
