...
const INFO_ENCRYPT_KEY = Buffer.from("paseto-encryption-key");
const INFO_AUTH_KEY = Buffer.from("paseto-auth-key-for-aead");
...
V4.prototype.splitKey = splitKey;
/**
 * @param {object} key
 * @param {Buffer} nonce
 */
function splitKey(key, nonce)
{
    const tmp = Buffer.from(sodium.crypto_generichash(
        56,
        Buffer.concat([INFO_ENCRYPT_KEY, nonce]),
        key.raw()
    ));
    ...
    const Ak = Buffer.from(sodium.crypto_generichash(
        32,
        Buffer.concat([INFO_AUTH_KEY, nonce]),
        key.raw()
    ));
    return {Ek, Ak, n2};
}
...
function aeadEncrypt(key, header, plaintext, footer, implicit, nonce) {
    // Step 2:
    if (nonce.length !== 32) {
        nonce = sodium.randombytes_buf(32);
    }
...
function aeadDecrypt(key, header, payload, footer, implicit) {
    ...
    const cipher = Buffer.from(payload).slice(32, plen - 32);
    const tag = Buffer.from(payload).slice(plen - 32);
